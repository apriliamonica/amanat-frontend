...
export default function IncomingMailTable({ mails, role }) {
  return (
      ...
      <table className="w-full">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">Dari</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">Perihal</th>
            <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">Prioritas</th>
          ...
          {mails.length > 0 ? (
            mails.map((mail) => (
              <tr key={mail.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 text-sm text-gray-600">{mail.from}</td>
                <td className="px-6 py-4 text-sm text-gray-900 max-w-xs truncate">{mail.subject}</td>
                <td className="px-6 py-4 text-sm">
                  <Badge variant={
                    mail.priority === 'Tinggi' ? 'danger' : 'warning'
                  }>
                    {mail.priority}
                  </Badge>
                </td>
                <td className="px-6 py-4 text-sm">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(mail.globalStatus, 'masuk')}`}>
